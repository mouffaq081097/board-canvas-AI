...
// Note: API key guard is at the top of POST handler, not here, to avoid breaking the route on init
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validTodos = todos.filter((t) => typeof t.text === 'string' && t.text.trim().length > 0);
    if (validTodos.length === 0) {
      return NextResponse.json({ error: 'No valid todo text provided' }, { status: 400 });
    }

    if (validTodos.length > 50) {
      return NextResponse.json({ error: 'Too many todos (max 50)' }, { status: 400 });
    }

    const todoList = validTodos.map((t, i) => `${i + 1}. ${t.text}`).join('\n');
